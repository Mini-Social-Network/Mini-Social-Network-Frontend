    ...
    <>
      <div style={{ width: '100%' }}>
        ...
        <Tabs activeKey={defaultActiveKey} onChange={onChange}>
          ...
          {isCurrent && (
            <Tabs.TabPane tab={t('mini.friend')} key="2">
              <FriendList />
            </Tabs.TabPane>
          )}
          {isExpert && (
            <Tabs.TabPane tab={t('mini.detailinfo')} key="3">
      ...
      <Modal title={t('mini.report')} visible={isOpenRP} onCancel={() => setIsOpenRP(false)} footer={<div />}>
        <BaseForm form={form} layout="vertical" name="contentForm" onFinish={handleReport}>
          <BaseForm.Item name="name" label={t('mini.resoneReport')} required>
