      ...
      <s.WrapperPost>
        {record.imageList.map((img: string) => (
          <s.ImgWrapper key={img}>
            <Image src={`http://localhost:8081/local-store/${img}`} width={100} height={100} preview={false}></Image>
          </s.ImgWrapper>
        ))}
  ...
  },
  {
    ...
    render: (record) => (
      <s.WrapperUser>
        <s.ImgWrapper>
          <Image
            src={record.user.imageUrl ? `http://localhost:8081/local-store/${record.user.imageUrl}` : dfavt}
            width={100}
            height={100}
            preview={false}
          ></Image>
