...
const FindPost: React.FC = () => {
  ...
  const [topicVal, setTopicVal] = useState<number>((state as number) ? parseInt((state as string).toString()) : 0);
  ...
  return (
    <>
      ...
      <s.Card title={t('mini.findPort')} bodyStyle={{ height: '48rem', padding: '20px 60px' }}>
        <Space wrap>
          <Search placeholder={t('mini.findpost')} enterButton onSearch={onSearch} />
